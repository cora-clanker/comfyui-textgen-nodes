...
``GET /textgen/models`` on ComfyUI's ``PromptServer``. The frontend reaches
it as ``/api/textgen/models`` and the ``Combo.Input``'s ``RemoteOptions``
points there.
...
@PromptServer.instance.routes.get("/textgen/models")
async def textgen_list_models(_request):
    ...
    try:
        ...
        _logger.warning("textgen: failed to list models: %s", exc)
        return web.json_response([])
